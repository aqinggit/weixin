<div class="weui-panel weui-panel_access weui-panel_hack" id="ajax-content-box">
    <?php if($columnInfo || $petInfo || $tagInfo){?>
    <div class="weui-panel__hd"><?php echo $columnInfo ? $columnInfo['title'] : ($petInfo ? $petInfo['title'] : $tagInfo['title']);?>文章</div>
    <?php }?>
    <div class="weui-panel__bd" id="ajax-content">
        <?php foreach($posts as $post){$this->renderPartial('/article/_item',array('data'=>$post));}?>
    </div>
    <?php if($loadMore){?>

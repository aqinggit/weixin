    ...
    static function minify_script_style($output, $has_tags = FALSE) {
        ...
        for ($i = count($chunks) - 1; $i >= 0; $i--) {
            $output = substr_replace(
                    $output, preg_replace('/\s*(:|;|,|}|{|\(|\))\s*/i', '$1', $chunks[$i][0]), $chunks[$i][1], strlen($chunks[$i][0])
            );
        }
        ...
        foreach ($array_output as $key => $value) {
            if ($in_string === FALSE && $in_dstring === FALSE) {
                if ($value === ' ') {
                    ...
                    if ((preg_match('/^[\x20-\x7f]*$/D', $next) OR preg_match('/^[\x20-\x7f]*$/D', $prev)) && (!ctype_alnum($next) OR ! ctype_alnum($prev)) && !in_array($next, array('\\', '_', '$'), TRUE) && !in_array($prev, array('\\', '_', '$'), TRUE)
                    ) {
        ...
        foreach ($lf as $feed_position) {
            foreach ($feed_position as $position) {
                ...
                if (!ctype_print($next) && !ctype_print($prev) && !preg_match('/^[\x20-\x7f]*$/D', $next) && !preg_match('/^[\x20-\x7f]*$/D', $prev)
                ) {
        ...
        return isset($open_tag) ? $open_tag . $output . $closing_tag : $output;
    }

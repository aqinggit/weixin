...
class cutpage {
    ...
    function cut_str() {
        ...
        if ($str_len_word <= $this->page_word) { //如果总字数小于一页显示字数 
            ...
            if (mb_stripos($this->pagestr, $this->cut_custom)) {
                $page_arr = explode($this->cut_custom, $this->pagestr);
            } else {
                $str_first = mb_substr($this->pagestr, 0, $this->page_word,'utf-8'); //0-page_word个文字 cutStr为func.global中的函数 
                foreach ($this->cut_tag as $v) {
                    $cut_start = mb_strripos($str_first, $v); //逆向查找第一个分页符的位置 
                    if ($cut_start) {
                        $page_arr[$i++] = mb_substr($this->pagestr, 0, $cut_start,'utf-8') . $v;
                        $cut_start = $cut_start + mb_strlen($v);
                        break;
                    }
                }
                if (($cut_start + $this->page_word) >= $str_len_word) { //如果超过总字数 
                    ...
                    while (($cut_start + $this->page_word) < $str_len_word) {
                        ...
                        foreach ($this->cut_tag as $v) {
                            $str_tmp = mb_substr($this->pagestr, $cut_start, $this->page_word,'utf-8'); //取第cut_start个字后的page_word个字符 
                            $cut_tmp = mb_strripos($str_tmp, $v); //找出从第cut_start个字之后，page_word个字之间，逆向查找第一个分页符的位置 
                            if ($cut_tmp) {
                                $page_arr[$i++] = mb_substr($str_tmp, 0, $cut_tmp,'utf-8') . $v;
                                $cut_start = $cut_start + $cut_tmp + mb_strlen($v);
                                break;
                            }
                        }
                    }
                    if (($cut_start + $this->page_word) > $str_len_word) {
                        $page_arr[$i++] = mb_substr($this->pagestr, $cut_start, $this->page_word,'utf-8');
                    }
                }
            }
        }

...
class zmf {
    ...
    public static function fp($msg, $arr = 0) {
        if ($arr) {
            file_put_contents(Yii::app()->basePath . '/runtime/log.txt', zmf::time().PHP_EOL.var_export($msg, true) . PHP_EOL, FILE_APPEND);
        } else {
            file_put_contents(Yii::app()->basePath . '/runtime/log.txt', zmf::time().PHP_EOL.$msg . PHP_EOL, FILE_APPEND);
        }
    }
    ...
    public static function config($type) {
        ...
        if (empty(Yii::app()->params['c'])) {
            ...
            static::writeSet($configs);
        } else {
            $configs = Yii::app()->params['c'];
        }
        if (in_array($type, array('domain', 'baseurl'))) {
            $host = $_SERVER['HTTP_HOST'];
            $mhost = $configs['mobileDomain']; //手机版网站
            $miphost = $configs['mipDomain']; //mip版网站
            $whost = $configs['domain'];//PC版网站
    ...
    public static function subStr($string, $sublen = 20, $separater = '...') {
        $start = 0;
        $string = static::stripStr($string);
        $string = static::trimText($string);
        $code = 'UTF-8';
        if ($code == 'UTF-8') {
            $string = strip_tags($string);
    ...
    public static function uploadDirs($ctime = '', $base = 'site', $type = 'posts') {
        ...
        $baseUrl = static::attachBase($base, $type);
        $_extra = static::getUpExtraUrl($ctime);
    ...
    public static function attachBase($base, $type = '') {
        if ($base === 'site') {
            //根据网站          
            if (static::config('imgVisitUrl') != '') {
                $baseUrl = static::config('imgVisitUrl') . '/';
            } else {
                $baseUrl = static::config('baseurl') . 'attachments/';
            }
            //如果是本地开发且是获取头像时，则不走云存储
            if (static::config('appStatus') == '1' && in_array($type, array('avatar'))) {
                $baseUrl = static::config('baseurl') . 'attachments/';
            }
        } elseif ($base === 'app') {
            //根据应用来
            if (static::config('imgUploadUrl') != '') {
                $baseUrl = static::config('imgUploadUrl') . '/';
            } else {
    ...
    public static function lazyImg() {
        return zmf::config('lazyImgUrl');
    }

    public static function getGravatar($email, $s = 240, $d = 'monsterid', $r = 'g') {
        return static::config('domain').'/common/images/default.png';
    }

    public static function getThumbnailUrl($url, $size = '') {
        if (!$url) {
            return '';
        }
        //c132|c120|c180|c105|c360
        //$appStatus = static::config('appStatus');
        $visitUrl = static::config('imgVisitUrl');
    ...
    public static function updateFCacheCounter($key, $value, $expire = 3600) {
        $_value = static::getFCache($key);
        if ($_value) {
            $value = (int) $value + (int) $_value;
        }
        static::setFCache($key, $value, $expire);
    }
    ...
    public static function filterInput($str, $textonly = 0) {
        if ($textonly == 1) {
            $str = static::cleanStr($str);
        } elseif ($textonly == 2) {
            $str = static::myint($str);
        }
        if ($textonly != 2) {
            $str = static::removeEmoji($str);
        }
        return $str;
    }
     ...
     * @return string
     */
    public static function cleanStr($string, $isurl = false) {
        $string = strip_tags($string);
        ...
        $string = static::fliterSql($string);
        return trim($string);
    }
     ...
     * @return string
     */
    public static function fliterSql($value) {
    ...
    public static function checkmobile(&$platform) {
        ...
        if (static::dstrpos($useragent, $pad_list)) {
            return false;
        }
        if (($v = static::dstrpos($useragent, $mobilebrowser_list, true))) {
            $platform = $v;
            return true;
        }
        $brower = array('mozilla', 'chrome', 'safari', 'opera', 'm3gate', 'winwap', 'openwave', 'myop');
        if (static::dstrpos($useragent, $brower)) {
            return false;
        }
        return false;
    }
    ...
    public static function checkSpider() {
        ...
        //判断蜘蛛
        $spider = null;
        if (stripos($ua, 'baiduspider') !== false || stripos($ua, 'baidumib') !== false) {
            $spider = 'baidu';
        }
        return $spider;
    }
    ...
    public static function formatNumber($number) {
        ...
        return number_format($number / 100000000, 2, '.', '') . '亿';
    }
     ...
     * @return type
     */
    public static function pinyin($string,$firstOnly=false) {
    ...
    public static function addcontentlink($content) {
        ...
        $str = preg_replace('/(((http|https):\/\/)[a-z0-9;&#@=_~%\?\/\.\,\+\-\!\:]+)/ie', "static::strip_link('$1')", $str);
        if (strpos($str, "http") === FALSE) {
            $str = preg_replace('/(www.[a-z0-9;&#@=_~%\?\/\.\,\+\-\!\:]+)/ie', "static::strip_link('$1')", $str);
        } else {
    ...
    public static function arrayDiff($array1, $array2) {
        ...
        foreach ($array1 as $k => $v) {
            if (!isset($array2[$k])) {
                ...
                $ret[$k] = static::arrayDiff($v, $array2[$k]);
     ...
     * @return  string 
     */
    public static function curlPost($url, $data = array(), $timeout = 30, $CA = false) {
    ...
    public static function text($params, $content, $lazyload = false, $size = 640) {
        if (!$content) {
            return $content;
        }
        if (is_array($params)) {
                ...
                foreach ($match[1] as $key => $val) {
                    ...
                    if ($src) {
                        ...
                        if ($lazyload) {
                            ...
                            //$imgurl = "<a href='javascript:;' action='zoom' action-type='img' action-data='{$_zoomImgUrl}'><img src='" . static::lazyImg() . "' class='lazy img-responsive' data-original='{$_imgurl}' " . ($action == 'edit' ? 'data="' . $val . '"' : '') . $_extra . "/></a>";
                            $imgurl = "<img src='" . static::lazyImg() . "' class='lazy img-responsive' data-original='{$_imgurl}' " . ($action == 'edit' ? 'data="' . $val . '"' : '') . "/>";
                        } else {
    ...
    public static function qrcode($content, $origin, $keyid) {
        ...
        $filename = $keyid . '.png';
        $siteUrl = static::config('baseurl') . 'attachments/qrcode/' . $origin . '/';
        $appUrl = Yii::app()->basePath . '/../attachments/qrcode/' . $origin . '/';
        static::createUploadDir($appUrl);
    ...
    public static function actionLimit($type, $keyid, $num = 4, $time = 60, $fileCache = false, $isCheck = false) {
        ...
        $info = static::myint(zmf::getCookie($cacheKey));
        if ($fileCache) {
            $cacheKey .= ip2long(Yii::app()->request->userHostAddress);
            $fileNum = static::myint(zmf::getFCache($cacheKey));
        }
    ...
    public static function urls($title, $main, $param, $opt = array()) {
        ...
        } elseif ($main == 'index/index') {
            $attr = array(
                $main,
                'colName' => zmf::val('colName', 1),
                'order' => zmf::val('order', 1),
            );
        }
        $attr[$key] = $value;
    ...
    public static function getNumberText($type) {
        $arr = array(
            '1' => '壹',
            '2' => '贰',
            '3' => '叁',
     ...
     * @param type $fontangle 角度
     * @param type $ttfpath 字体名称
     * @param type $char 字符
     * @return type
     */
    ...
    public static function url($params=array()) {
        ...
        $parse=parse_url($url);
        $path= static::config('domain').$parse['path'];
        $arr= tools::parseUrlParams($url);
        foreach ($params as $_k=>$v){
            $arr[$_k]=$v;
        }
        $str='';
    ...
    public static function curlSogouWenwen($url) {
        $ch = curl_init();
        // 设置浏览器的特定header
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            ...
            'Cookie:ww_sTitle=%E5%93%88%E5%93%88; IPLOC=CN5000; SUID=939ACD761808990A0000000059CE19B2; SUV=1506679218953597; sw_uuid=5640253944; dt_ssuid=5463645264; pex=C864C03270DED3DD8A06887A372DA219231FFAC25A9D64AE09E82AED12E416AC; ssuid=9499474000; ld=8kllllllll2B8PXRlllllVXnptGlllllBUj8Myllll9lllll4ylll5@@@@@@@@@@',
        ));
...
//        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
     ...
     * @return array
     */
    public static function calAge($birthday, $string = true) {
        if (is_numeric($birthday)) {
            $birthday = zmf::time($birthday, 'Y-m-d');
        }
        ...
        if ($string) {
            return ($years > 0 ? $years . '岁' : '') . ($m > 0 ? $m . '个月' : '') . ($d > 0 ? $d . '天' : '');
        }
     ...
     * @param type $bymd
     */
    private function getMD($ymd, $bymd) {
        list($y, $m, $d) = $ymd;
        list($by, $bm, $bd) = $bymd;
        if (($m . $d) < ($bm . $bd)) {
            $m += 12;
        ...
        if ($bd <= $d) {//同处一个月
            ...
            $mdays = $bm > 12 ? self::_getMothDay( ++$by, $bm - 12) : self::_getMothDay($by, $bm);
            $day = $mdays - $bd + $d;
        }
     ...
     * @return string
     */
    public static function link($text, $url = '#', $htmlOptions = array(),$domain=null) {
        if (is_array($url)) {
            $_tmp = $url[0];
            unset($url[0]);
            $_url = static::createUrl($_tmp, $url,$domain);
        } else {
            $_url = $url;
        }
        $htmlOptions['title'] = strip_tags(!isset($htmlOptions['title']) ? $text : $htmlOptions['title']);
        return CHtml::link($text, $_url, $htmlOptions);
    }
     ...
     * @return string
     */
    public static function createUrl($route, $params = array(),$domain=null) {
        return zmf::config($domain ? $domain : 'domain') . Yii::app()->createUrl($route, $params);
    }
    ...
    public static function dbAll($sql) {
    ...
    public static function rolling_curl($urls, $delay, $referer = '') {
        ...
        do {
            ...
            while ($done = curl_multi_info_read($queue)) {
                ...
                $results = static::caiji_callback(curl_multi_getcontent($done['handle']), $delay);
    ...
    public static function caiji_callback($data, $delay) {
    ...
    public static function filterIds($ids){
        ...
        $arr=array_unique(array_filter(explode(',', $ids)));
        foreach ($arr as $k=>$id){
            if(!is_numeric($id)){
                unset($arr[$k]);
            }
    ...
    public static function mipImg($url,$alt='',$htmlOptions=array()){
        if(!$url){
            return '';
        }
        $htmlOptions['src']=$url;
        $htmlOptions['alt']=$alt;
        return CHtml::tag('mip-img',$htmlOptions);
    }

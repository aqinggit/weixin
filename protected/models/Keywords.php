...
class Keywords extends CActiveRecord {
    ...
    public static function linkWords($content,$from='keywords') {
        $keywords = static::getWords();  
        if(empty($keywords) || !$keywords){
            return $content;
        }
        $isMobile= zmf::checkmobile($platform);
        $domain=zmf::config('domain');
        $staticDomain=zmf::config('staticDomain');
        //处理link
        $replace = array(
            "/\[link=([^\]]+?)\](.+?)\[\/link\]/i",
        );
        $to = array(
            '<a href="\\1" '.(!$isMobile ? 'target="_blank"' : '').' class="_auto">\\2</a>',
        );
        ...
        //取所有图片  
        preg_match_all('/<mip\-img.*?src=[\'|\"].*?[\'|\"].*?>.*?<\/mip\-img>/i', $content, $mipImgList);
        $content = preg_replace('/<mip\-img.*?src=[\'|\"].*?[\'|\"].*?>.*?<\/mip\-img>/i', '<{mipImg}>', $content);  
        
        preg_match_all('/<[img|IMG].*?src=[\'|\"].*?[\'|\"].*?[\/]?>/i', $content, $imgList);
        $content = preg_replace('/<[img|IMG].*?src=[\'|\"].*?[\'|\"].*?[\/]?>/i', '<{tupian}>', $content);
            ...
            if (strpos($content, $title) !== false) {
                ...
                if (strpos($content, $_arrkey) !== false) {
                    ...
                    if (!empty($_matchtmp[0])) {
                        ...
                        if($staticDomain!=$domain){
                            $url=str_replace($staticDomain,$domain,$url);
                        }
                        $guideList[$_arrkey] = '<a href="'.$url.'" title="'.$title.'"><strong>'.$title.'</strong></a>';
                    }
                }
            }
        }
        ...
        $arrLen2 = count($mipImgList[0]);
        for ($i = 0; $i < $arrLen2; $i++) {
            $content = preg_replace('/<{mipImg}>/', $mipImgList[0][$i], $content, 1);
        }
        $arrLen2 = count($imgList);
        for ($i = 0; $i < $arrLen2; $i++) {
            $content = preg_replace('/<{tupian}>/', $imgList[$i], $content, 1);
        }
    ...
    public static function linkHtml($html) {
        $keywords = static::getWords();  
        if(empty($keywords) || !$keywords){
            return $html;
        }
        ...
        $isMobile= zmf::checkmobile($platform);
        //处理script
        preg_match_all('/<script.*?>.*?<\/script>/si', $content, $scriptList);
        $content = preg_replace('/<script.*?>.*?<\/script>/si', '<{script}>', $content);
        //处理style
        preg_match_all('/<style.*?>.*?<\/style>/si', $content, $styleList);
        $content = preg_replace('/<style.*?>.*?<\/style>/si', '<{style}>', $content);
        ...
        //处理所有链接
        preg_match_all('/<a.*?href=[\'|\"].*?[\'|\"].*?>.*?<\/a>/si', $content, $linkList);
        $content = preg_replace('/<a.*?href=[\'|\"].*?[\'|\"].*?>.*?<\/a>/si', '<{link}>', $content);        
        ...
        foreach ($keywords as $title => $url) {
            if (strpos($content, $title) !== false) {
                ...
                if (strpos($content, $_arrkey) !== false) {
                    ...
                    if (!empty($_matchtmp[0])) {
                        $content = preg_replace($find, $_arrkey, $content, 1);
                        $guideList[$_arrkey] = '<a href="'.$url.'" '.(!$isMobile ? 'target="_blank"' : '').' title="'.$title.'"><strong>'.$title.'</strong></a>';
                    }
                }
            }
        }
        $len = count($scriptList[0]);
        for ($i = 0; $i < $len; $i++) {
            $content = preg_replace('/<{script}>/', $scriptList[0][$i], $content, 1);
        }
        
        $len = count($styleList[0]);
        for ($i = 0; $i < $len; $i++) {
            $content = preg_replace('/<{style}>/', $styleList[0][$i], $content, 1);
        }
        ...
        $content = str_replace(array('\(', '\)', '\/', '\.'), array('(', ')', '/', '.'), $content);
        //$content = minify::compress($content);       
        $html= str_replace($matches[0], $content, $html);
        return $html;
    }

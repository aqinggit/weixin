...
class Words extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('type, len,status,action', 'numerical', 'integerOnly' => true),
            array('word,replaceTo', 'length', 'max' => 255),
            array('word', 'unique'),
    ...
    public static function exActions($type) {
        $arr = array(
            static::ACTION_BANED => '禁止',
            static::ACTION_REPLACE => '替换',
        );
        if ($type == 'admin') {
            return $arr;
        }
    ...
    public static function createWordsCache() {
        $items = static::model()->findAll(array(
            'condition' => 'action=' . static::ACTION_BANED,
            'order' => 'len DESC',
            'select' => 'word'
        ));

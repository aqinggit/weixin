...
class Attachments extends CActiveRecord {
    ...
    public static function updateUrlPrefix($id, $info = null) {
        ...
        if (!$info || (!$info['typeId']) || $info['status']!=Users::STATUS_PASSED) {
            return;
    ...
    public static function updatePostImgs($logid, $classify, $attachids, $uid = 0) {
        ...
        Attachments::model()->updateAll(array('status' => Users::STATUS_DELED), 'logid=:logid AND classify=:classify', array(':logid' => $logid, ':classify' => $classify));
        if ($attachids != '') {
            Attachments::model()->updateAll(array('status' => Users::STATUS_PASSED, 'logid' => $logid), "id IN({$attachids}) AND classify='{$classify}'");
        }
        return true;
    }

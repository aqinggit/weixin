...
class TagRelation extends CActiveRecord {
    ...
    public function checkAndWriteTag($id, $crumb, $tagid = 0) {
        ...
        $reinfo = TagRelation::model()->find('logid=:logid AND tagid=:tagid AND classify="'.Column::CLASSIFY_POST.'"', array(':logid' => $id, ':tagid' => $_tagid));
        if (!$reinfo) {
    ...
    public static function addRelation($attr) {
        if (!$attr) {
            return false;
        }
        $reinfo = TagRelation::model()->find('logid=:logid AND tagid=:tagid AND classify=:class', array(':logid' => $attr['logid'], ':tagid' => $attr['tagid'], ':class' => $attr['classify']));
        if (!$reinfo) {
            $model_tagrel = new TagRelation;
            $model_tagrel->attributes = $attr;
            return $model_tagrel->save();
        }
        return true;
    }

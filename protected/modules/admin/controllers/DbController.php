...
class DbController extends Admin {
...
//        $this->checkPower('db');
    ...
    private function exportDatabase($tables, $sqlcompat, $sqlcharset, $sizelimit, $fileid, $random, $tableid, $startfrom, $tabletype) {
        ...
        if (trim($tabledump)) {            
            ...
            $this->message(1, "备份文件 $filename 写入成功!", $this->createUrl('db/doExport', array('sizelimit' => $sizelimit, 'tableid' => $tableid, 'fileid' => $fileid, 'random' => $random, 'startfrom' => $startrow, 'dosubmit' => 1, 'sqlcompat' => $sqlcompat, 'sqlcharset' => $sqlcharset, 'tabletype' => $tabletype)), 1);
        } else {
        ...
        if ($this->_gets->getParam('dosubmit')) {
            ...
            if (is_array($sqlfiles)) {
                ...
                foreach ($sqlfiles as $id => $sqlfile) {                    
                    if (preg_match("/(zmfcms_[0-9]{8}_[0-9a-z]{4}_)([0-9]+)\.sql/i", basename($sqlfile), $num)) {
    ...
    private function _import($filename, $fileid) {
        if ($filename && CFileHelper::getExtension($filename) == 'sql') {
            ...
            if (file_exists($filepath)) {
                ...
                $this->message(1, '数据文件' . $filename . ' 导入成功', $this->createUrl('db/import', array('pre' => $pre, 'fileid' => $fileid, 'dosubmit' => '1')), 1);
            } else {
                $this->message(1, '数据成功导入', $this->createUrl('db/import'), 3);
            }
        }
    }
    ...
    private function _execute($command = '') {
        ...
        $formatExeSql = self::splitsql($exeSql);
    ...
    private function _executeCommand($sql = '') {
        ...
        if (preg_match('/^\s*"?(' . $queryType . ')\s+/i', $sql)) {
            $data['result'] = Yii::app()->db->createCommand($sql)->execute();
            $data['type'] = 'execute';
        } else {
            ...
            case 'deleteFile':                
                ...
                if ($filenames) {
                    if (is_array($filenames)) {
                        ...
                        $this->message(1, '删除完成', $this->createUrl('db/import'));
                    } else {
                    ...
                    $this->message(0, '请选择要删除的文件', $this->createUrl('db/import'));
                }
                break;
            case 'downloadFile':                
